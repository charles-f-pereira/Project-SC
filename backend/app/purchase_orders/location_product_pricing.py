...
Also: alternatePrimaryFlag (Y/N) for TempActivateAltPrimary products (alternate vendor products).
"""
...
def _build_alt_primary_payload(
    location_code: str, product_numbers: List[str], alternate_primary_flag: str
) -> dict:
    """Build request body for saveLocationProductPricing (alternatePrimaryFlag)."""
    units = [
        {
            "alternatePrimaryFlag": alternate_primary_flag,
            "locationCode": location_code,
            "productNumber": pn,
        }
        for pn in product_numbers
...
def save_location_product_pricing_alt_primary_sync(
    location_code: str,
    product_numbers: List[str],
    alternate_primary_flag: str,
) -> Tuple[bool, int, str]:
    """
    Call Crunchtime saveLocationProductPricing with alternatePrimaryFlag (Y/N).
    Used before savePurchaseOrders for TempActivateAltPrimary products, then deactivate after.
    Returns (success, status_code, message).
    """
    location_code = (location_code or "").strip()
    if not location_code or not product_numbers:
    ...
    if not product_numbers:
        return True, 200, "No products to update"
    flag = "Y" if (alternate_primary_flag or "").strip().upper() == "Y" else "N"
    payload = _build_alt_primary_payload(location_code, product_numbers, flag)
    token = service_token("locationproductpricing")
    headers = {
        **ct_headers(token_override=token),
        "accept": "application/json",
    ...
    logger.info(
        "saveLocationProductPricing (alt primary): location=%s products=%s alternatePrimaryFlag=%s",
        location_code,
        product_numbers,
        flag,
    )
    try:
        ...
        if r.status_code == 200:
            logger.info(
                "saveLocationProductPricing (alt primary): success location=%s alternatePrimaryFlag=%s",
                location_code,
                flag,
            )
            return True, r.status_code, r.text or ""
        logger.warning(
            "saveLocationProductPricing (alt primary): failed location=%s status=%s body=%s",
            location_code,
            r.status_code,
            (r.text or "")[:500],
        )
        ...
        logger.exception(
            "saveLocationProductPricing (alt primary): error location=%s: %s",
            location_code,
            e,
        )
        return False, 0, str(e)
