...
function getEventColor(event, view) {
  ...
  if (r?.type === 'order') {
    return { bg: orderPalette[idx % orderPalette.length], border: r?.isOverride ? '2px dashed #1a252f' : 'none' }
  }
  if (r?.type === 'delivery') {
    return { bg: deliveryPalette[idx % deliveryPalette.length], border: r?.isOverride ? '2px dashed #0e6655' : 'none' }
  }
...
function findPairedEvent(selected, events) {
  ...
  const isSameVendorLoc = (er) =>
    er && er.type !== 'holiday' &&
    er.locationCode === loc && er.vendorCode === vend && er.orderDay === orderDay && !!er.isOverride === isOverride
  ...
  if (type === 'order') {
    ...
    return events.find((e) => {
      ...
      return sameDate(e.start, orderDeliveryDate) || (er.deliveryDate != null && sameDate(er.deliveryDate, orderDeliveryDate))
    }) || null
  }
...
export default function ScheduleCalendar({ schedules, holidays, filters }) {
  ...
  const scheduleEvents = useMemo(() => {
    ...
    console.log('[ScheduleCalendar] Schedules → events:', { scheduleCount: (schedules || []).length, eventCount: events.length, sampleEvent: events[0] })
    return events
  }, [schedules])
    ...
    <div className="schedule-calendar">
      <div className="calendar-header">
        ...
        <div className="calendar-header-right">
          ...
          <span className="legend-item">
            <span className="legend-color" style={{ backgroundColor: VENDOR_ORDER_COLORS[0] }}></span>
            Order By (std)
          </span>
          <span className="legend-item">
            <span className="legend-color" style={{ backgroundColor: VENDOR_DELIVERY_COLORS[0] }}></span>
            Delivery (std)
          </span>
      ...
      {schedules && schedules.length > 0 && scheduleEvents.length === 0 && (
        <div className="calendar-empty calendar-empty-hint">
          <p>Schedule data received ({schedules.length} vendor/location) but no calendar events were generated.</p>
