...
export default function Dashboard() {
  ...
  const filteredHolidays = useMemo(() => {
    ...
    return holidays.filter((h) =>
      codesForSelected.has(h.country || (h.Country || '').toUpperCase()),
    );
  ...
  const fetchPublicHolidays = async () => {
    try {
      ...
      setHolidaysError(
        err.response?.data?.detail || err.message || 'Failed to fetch public holidays',
      );
    } finally {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      const res = await client.get('/api/schedules/vendor-locations', {
        params: scheduleFetchParams,
      });
      let data = res.data?.data || [];
      console.log('[View Schedules] API response:', {
        count: data.length,
        firstKeys: data[0] ? Object.keys(data[0]) : [],
        sample: data[0],
      });
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-calendar-section">
          <div className="calendar-actions">
            ...
            {holidaysError && <span className="holidays-error">{holidaysError}</span>}
            {schedulesError && <span className="holidays-error">{schedulesError}</span>}
          </div>
          <ScheduleCalendar
