...
const ORDER_DAY_NAMES = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
const ORDER_DAY_ABBR = { Monday: 'Mon', Tuesday: 'Tue', Wednesday: 'Wed', Thursday: 'Thu', Friday: 'Fri', Saturday: 'Sat', Sunday: 'Sun' }
const DELIVERY_DAY_NAMES = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
...
function getVal(obj, ...keys) {
  for (const k of keys) {
    ...
    if (lower !== k) { v = obj[lower]; if (v !== undefined && v !== null) return v }
  }
  return undefined
}
...
const ORDER_DAY_TO_JS_DAY = { Monday: 1, Tuesday: 2, Wednesday: 3, Thursday: 4, Friday: 5, Saturday: 6, Sunday: 0 }
...
function processDetailArray(events, item, details, startDate, endDate, isOverride, vendorIndex, totalVendors) {
  ...
  for (const detail of details) {
    for (const dayName of ORDER_DAY_NAMES) {
      ...
      const deliverKeyAbbr = ORDER_DAY_ABBR[dayName] ? `deliverDay${ORDER_DAY_ABBR[dayName]}` : null

      const orderFlag = getVal(detail, flagKey, `orderDay${dayName}flag`)
      const orderFlagStr = String(orderFlag || '').toLowerCase()
      const hasOrderDay = !!(orderFlag === true || orderFlag === 1 || orderFlagStr === 'true' || orderFlagStr === 'y')
      ...
      const hasDeliveryDay = deliverDayVal != null && !Number.isNaN(deliverDayVal) && deliverDayVal >= 1 && deliverDayVal <= 21
      ...
      if (hasOrderDay) {
        ...
        while (cursor <= endDate && iter < 104) {
          ...
          if (hasDeliveryDay && deliveryDate && deliveryDate >= startDate && deliveryDate <= endDate) {
            const { hours, minutes } = parseTime(orderByTime)
            const deliveryStart = new Date(deliveryDate.getFullYear(), deliveryDate.getMonth(), deliveryDate.getDate(), hours, minutes, 0, 0)
        ...
        for (let w = 0; w < 104; w++) {
          ...
          if (deliveryDate && deliveryDate >= startDate && deliveryDate <= endDate) {
            events.push({
              ...
              title: `${prefix}Delivery · ${vendorCode} (${deliveryLabel})`,
              start: new Date(deliveryDate.getFullYear(), deliveryDate.getMonth(), deliveryDate.getDate()),
              end: new Date(deliveryDate.getFullYear(), deliveryDate.getMonth(), deliveryDate.getDate()),
              allDay: true,
              resource: {
                type: 'delivery',
                locationCode,
...
export function scheduleToEvents(scheduleItems, options = {}) {
  ...
  const startDate = options.startDate ? new Date(options.startDate) : new Date(today.getFullYear(), today.getMonth() - 3, 1)
  startDate.setHours(0, 0, 0, 0)
  const endDate = options.endDate ? new Date(options.endDate) : new Date(today.getFullYear(), today.getMonth() + 4, 0)
  ...
  scheduleItems.forEach((item, vendorIndex) => {
    ...
    let standardDetails = getVal(item, 'vendorLocationScheduleDetail', 'VendorLocationScheduleDetail')
    ...
    processDetailArray(events, item, standardDetails, startDate, endDate, false, vendorIndex, totalVendors)
    if (overrideDetails.length) {
      processDetailArray(events, item, overrideDetails, startDate, endDate, true, vendorIndex, totalVendors)
    }
  })
